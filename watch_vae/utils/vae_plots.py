...
def mnist_test_tsne(input_scale, vae=None, test_loader=None):
    """
    ...
    if 1:
        ...
        # Upscale.
        data = torch.repeat_interleave(data, input_scale, dim=2)
        data = torch.repeat_interleave(data, input_scale, dim=3)

    mnist_labels = test_loader.dataset.test_labels
    z_loc, z_scale = vae.encoder(data)

    if 1:
        z_loc = z_loc.reshape(z_loc.shape[0], -1)
