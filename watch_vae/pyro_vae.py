...
"""
TODO:
* Make a 2D conv VAE. There's probably no point doing the 1D version.
"""
...
class EncoderUnet(nn.Module):
    def __init__(self, z_dim, hidden_dim, n_channels, n_classes, bilinear=False):
        super().__init__()
        # setup the three linear transformations used
        # self.fc1 = nn.Linear(784, hidden_dim)
        # self.fc21 = nn.Linear(hidden_dim, z_dim)
        ...
        self.z_loc_encode = nn.Conv2d(1024 // factor, 1024 // factor, kernel_size=3, padding=1, bias=False)
        self.z_scale_encode = nn.Conv2d(1024 // factor, 1024 // factor, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        z_loc = self.z_loc_encode(x5)
        z_scale = torch.exp(self.z_scale_encode(x5))

        # TODO: Linear layer/s to transform x5 to the z_dim.
        # TODO: Predict loc and scale.
...
class DecoderUnet(nn.Module):
    def __init__(self, z_dim, hidden_dim, bilinear=False):
        super().__init__()
        # setup the two linear transformations used
        # self.fc1 = nn.Linear(z_dim, hidden_dim)
        # self.fc21 = nn.Linear(hidden_dim, 784)
...
class VAEConv1D(nn.Module):
    ...
    def __init__(self, z_dim=50, hidden_dim=400, use_cuda=False):
        super().__init__()
        # create the encoder and decoder networks
        self.encoder = EncoderConv1D(z_dim, hidden_dim)
        self.decoder = Decoder(z_dim, hidden_dim)
    ...
    def guide(self, x):
        ...
        with pyro.plate("data", x.shape[0]):
            # use the encoder to get the parameters used to define q(z|x)
            z_loc, z_scale = self.encoder.forward(x.reshape((-1, 1, 784)))
            # sample the latent code z
            pyro.sample("latent", dist.Normal(z_loc, z_scale).to_event(1))
...
def main(args):
    ...
    if not conv_vae:
        ...
        vae = VAEConv1D(use_cuda=args.cuda)
    ...
    for epoch in range(args.num_epochs):
        ...
        for x, _ in train_loader:
            ...
            if conv_vae:
                pass
                #x = x.reshape((-1, 1, 784))  # 1d conv
                #x = x.reshape((-1, 1, 28, 28))  # 2d conv
